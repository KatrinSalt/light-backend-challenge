...
func TestWorkflowRuleStore_GetByCompanyID(t *testing.T) {
	tests := []struct {
		name  string
		input struct {
			store     *workflowRuleStore
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, gotErr := test.input.store.GetByCompanyID(test.input.companyID)

			if test.wantErr {
				if gotErr == nil {
					t.Errorf("GetByCompanyID() expected error but got none")
					return
				}
				if test.errMsg != "" && gotErr.Error() != test.errMsg {
					t.Errorf("GetByCompanyID() expected error %q but got %q", test.errMsg, gotErr.Error())
				}
				return
			}

			if gotErr != nil {
				t.Errorf("GetByCompanyID() unexpected error: %v", gotErr)
				return
			}

			if diff := cmp.Diff(test.want, got); diff != "" {
				t.Errorf("GetByCompanyID() mismatch (-want +got)\n%s", diff)
			}
		})
	}
}
